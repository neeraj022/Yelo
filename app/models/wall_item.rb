class WallItem
  ...
  def save_tag_user(wall, usr)
    number = User.mobile_number_format(usr[:mobile_number])
  ...
  def send_wall_tag_sms(wall, num)
    @mobile_app_url ||= AppSetting.mobile_app_url
    mobile_number = number[:mobile_number]
    country_code = set_country_code(num[:country_code])
